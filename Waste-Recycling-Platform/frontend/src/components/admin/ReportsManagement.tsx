                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {report.formattedImages?.map((img: string, i: number) => (
                      <button type="button" key={i} onClick={() => setGalleryOpen(true)} className="aspect-square rounded-lg cursor-pointer hover:opacity-80 border overflow-hidden">
                        <img src={img} alt={`Ảnh báo cáo ${i + 1}`} className="w-full h-full object-cover" />
                      </button>
                    ))}
                  </div>
                </div>
