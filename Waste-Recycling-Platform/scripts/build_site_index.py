...
def build_owner_cards(owners: list[str], owner_base: str, published_root: Path) -> str:
    ...
    for owner in owners:
        slug = slugify(owner)
        owner_base_url = f'{owner_base}owners/{slug}/'
        owner_index = f'{owner_base}owners/{slug}/index.html'
        owner_pdf = f'{owner_base}owners/{slug}/report.pdf'
        owner_pdf_exists = (published_root / 'report-extra' / 'owners' / slug / 'report.pdf').exists()
        # Buttons: Open HTML, Download HTML, optional PDF if present
        pdf_button = f'<a class="btn mini ghost" href="{owner_pdf}">Tải PDF</a>' if owner_pdf_exists else ''
        cards.append(
          f'<div class="owner-card">'
            f'<a class="owner-link" href="{owner_base_url}">'
              f'<span class="owner-name">{owner}</span>'
              f'<span class="owner-sub">Mở báo cáo theo owner</span>'
            f'</a>'
            f'<div class="owner-actions">'
              f'<a class="btn mini ghost" href="{owner_index}">Mở HTML</a>'
              f'<a class="btn mini ghost" href="{owner_index}" download>Download HTML</a>'
              {pdf_button}
            f'</div>'
          f'</div>'
        )
    ...
    <section class="hero">
      <div class="panel hero-main">
        ...
        <div class="actions">
          ...
          <a class="btn ghost" href="{report_url}index.html">Open main HTML</a>
          <a class="btn ghost" href="{report_url}index.html" download>Download main HTML</a>
